...
function createPages() {
  ...
  for (let n = 1; n <= totalPages; n += 1) {
    ...
    image.fetchPriority = n <= 2 ? "high" : "low";
...
function updateLoading() {
  loadedImages += 1;
  const target = Math.min(2, totalPages);
...
function flipPrevious() {
  if (!pageFlip || zoomLevel > 1) return;
  pageFlip.flipPrev();
}
...
$("pagesButton").addEventListener("click", () => {
  ...
  setPanel("thumbnailPanel", willOpen);
  if (willOpen) loadThumbnailImages();
});
...
/* Two-finger pinch zoom only.
   One-finger gestures are handled entirely by StPageFlip so the page-curl
   animation works correctly in both directions on mobile. */
