...
if (edgePrevious) edgePrevious.addEventListener("click", () => {
  if (isTouchDevice && mobileSwipeLocked) return;
  if (isTouchDevice) mobileSwipeLocked = true;
  pageFlip.flipPrev();
  if (isTouchDevice) unlockMobileSwipe();
});
if (edgeNext) edgeNext.addEventListener("click", () => {
  if (isTouchDevice && mobileSwipeLocked) return;
  if (isTouchDevice) mobileSwipeLocked = true;
  pageFlip.flipNext();
  if (isTouchDevice) unlockMobileSwipe();
...
/* Clean mobile swipe controller.
   StPageFlip's own touch handling is disabled on coarse-pointer devices,
   so only this handler decides whether the page turns forwards or backwards. */
let swipeStartX = 0;
let swipeStartY = 0;
...
if (isTouchDevice && bookStage) {
  bookStage.addEventListener(
    "touchstart",
    (event) => {
      if (event.touches.length !== 1 || mobileSwipeLocked) return;
  ...
  bookStage.addEventListener(
    "touchend",
    (event) => {
      if (!pageFlip || mobileSwipeLocked || event.changedTouches.length !== 1) return;
